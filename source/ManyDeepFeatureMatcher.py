...
class ManyDeepFeatureMatcher(torch.nn.Module):
    ...
    def extract_singlescale(self, img_A, img_B, display_results=0, *args):
        ...
        if self.USE_Layer5:
            descriptorsA = torch.concat([descriptorsA, activations_A[4][0][0:SHRINK_DIM, np.int64(detectionA[:, 0]/16), np.int64(detectionA[:, 1]/16)]], dim=0)
            descriptorsB = torch.concat([descriptorsB, activations_B[4][0][0:SHRINK_DIM, np.int64(detectionB[:, 0]/16), np.int64(detectionB[:, 1]/16)]], dim=0)
