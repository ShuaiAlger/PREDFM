...
class ParamMatcher(torch.nn.Module):
    def __init__(self, model: str = 'resnet18', device = None,
        ...
        self.combination_nums = 0

        self.combination_index = 0

        self.return_nodes_list = []

        # for i1 in range(len(self.layers[0])):
        #     for i2 in range(len(self.layers[1])):
        #         for i3 in range(len(self.layers[2])):
        #             for i4 in range(len(self.layers[3])):
        #                     self.return_nodes = {
        #                         self.layers[0][i1]: self.layers[0][i1],
        #                         self.layers[1][i2]: self.layers[1][i2],
        #                         self.layers[2][i3]: self.layers[2][i3],
        #                         self.layers[3][i4]: self.layers[3][i4],
        #                     }
