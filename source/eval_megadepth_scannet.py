...
def get_megadepth_loader():
    ...
    for npz_name in tqdm(npz_names):
        ...
        # sets.append(MegaDepthDataset(root_dir=data_root_dir, npz_path=npz_path, mode='test', min_overlap_score=0.0, img_resize=800, img_padding=False, depth_padding=False, df=8))
...
def evaluate_megadepth(matcher=None):
    ...
    with tqdm(total=len(datas)) as _tqdm:
        for data in tqdm(datas):
            ...
            mkpts0_copy = copy.deepcopy(mkpts0.copy())
            ...
            cv2.imwrite("canvas_res/"+str(counts)+".png", draw_green_red(img0, img1, mkpts0_copy, mkpts1_copy, correct, 1))
            counts = counts + 1
            ...
            # print(pose_error, precisions, matching_score)
