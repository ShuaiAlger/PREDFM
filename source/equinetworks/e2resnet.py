...
class E2ResNet(torch.nn.Module):
    ...
    def forward_multi_features(self, x: Tensor) -> GeometricTensor:

        x = GeometricTensor(x, self.in_type)
        x1 = self.conv1(x)
        x1b = self.bn1(x1)
        x1r = self.relu1(x1b)
        x1p = self.maxpool(x1r)
        ...
        USE_GPool4 = 0
        if USE_GPool4:
            x1r = nn.GroupPooling(FieldType(self.gspace,  39 * [self.gspace.regular_repr]))(x1r)
            x1l = nn.GroupPooling(FieldType(self.gspace,  39 * [self.gspace.regular_repr]))(x1l)
            x2 = nn.GroupPooling(FieldType(self.gspace,  78 * [self.gspace.regular_repr]))(x2)
            x3 = nn.GroupPooling(FieldType(self.gspace,  156 * [self.gspace.regular_repr]))(x3)
            x4 = nn.GroupPooling(FieldType(self.gspace,  312 * [self.gspace.regular_repr]))(x4)

        USE_GPool8 = 0
        if USE_GPool8:
            x1r = nn.GroupPooling(FieldType(self.gspace,  28 * [self.gspace.regular_repr]))(x1r)
            x1l = nn.GroupPooling(FieldType(self.gspace,  28 * [self.gspace.regular_repr]))(x1l)
            x2 = nn.GroupPooling(FieldType(self.gspace,  56 * [self.gspace.regular_repr]))(x2)
            x3 = nn.GroupPooling(FieldType(self.gspace,  112 * [self.gspace.regular_repr]))(x3)
            x4 = nn.GroupPooling(FieldType(self.gspace,  224 * [self.gspace.regular_repr]))(x4)

        # return [x1r, x1p, x1l, x2, x3, x4]
        # return [x1r, x1l, x2, x3, x4]
        return [x1r, x1l, x2, x3, x4]
